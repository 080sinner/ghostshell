...
/*   Updated: 2021/11/17 23:05:48 by eozben           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		handle_quotes(char *input, t_tok *current)
{
	...
	while (*input != '\0')
	{
		if (*input == SQUOTE)
			...
			state[DQUOTE] = ON;
		while (state[SQUOTE] == ON || state[DQUOTE] == ON)		/* was passiert hier ueberhaupt 
																schreib die scheisse mal fertig wenn du vor mir da bist	*/
		{
			len++;
			input++;
			if (*input == SQUOTE)
